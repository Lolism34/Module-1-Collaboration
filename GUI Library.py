...
def search_media():
    ...
    if search_term:
        ...
        if results:
            ...
            # Add a Scrollbar and Listbox to the window
            scrollbar = Scrollbar(result_window)
            scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

            listbox = Listbox(result_window, width=100, height=20, yscrollcommand=scrollbar.set)
            listbox.pack()

            # Insert search results into the Listbox
            for result in results:
                media_str = f"ID: {result[0]}, Title: {result[1]}, Author: {result[2]}, Genre: {result[3]}, Available: {'Yes' if result[6] == 1 else 'No'}"
                listbox.insert(tk.END, media_str)

            scrollbar.config(command=listbox.yview)

            # Add a function to display details of selected media
            def show_media_details(event):
                selection = listbox.curselection()
                if selection:
                    selected_media = listbox.get(selection[0])
                    messagebox.showinfo("Media Details", selected_media)

            # Bind selection event
            listbox.bind("<<ListboxSelect>>", show_media_details)
